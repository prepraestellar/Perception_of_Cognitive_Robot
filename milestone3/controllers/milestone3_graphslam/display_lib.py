...
class DisplayManager:
    ...
    def __init__(self, display, global_display, motion_display=None):
        ...
        # Fixed global transform prevents visual warping from per-frame rescaling.
        self._global_scale = 2.0  # pixels per grid cell
        self._global_cx = self.gdisplay_w // 2
        self._global_cy = self.gdisplay_h // 2
    ...
    def draw_global_map(self, graph_map, rx, ry, frontier_set):
        ...
        for (gx, gy), occ in graph_map.nodes.items():
            px = int(self._global_cx - gy * scale)
            py = int(self._global_cy - gx * scale)
        ...
        rpx = int(self._global_cx - rgy * scale)
        rpy = int(self._global_cy - rgx * scale)
        dot = max(sz * 3, 5)
        self.global_display.setColor(0xFF0000)
        self.global_display.fillRectangle(rpx - dot // 2, rpy - dot // 2, dot, dot)

    def draw_slam_nodes_local(self, graph_map, slam_poses, rx, ry):
        """
        Draw SLAM keyframe nodes on local map (cyan circles).
        
        Args:
            graph_map: GraphMap instance
            slam_poses: List of [x, y, theta] SLAM poses
            rx, ry: Robot position in world coordinates
        """
        robot_gx, robot_gy = graph_map.world_to_grid(rx, ry)
        cx, cy = self.display_w // 2, self.display_h // 2
        ...
        for pose in slam_poses:
            node_gx, node_gy = graph_map.world_to_grid(pose[0], pose[1])
            rel_x = node_gx - robot_gx
            rel_y = node_gy - robot_gy
            px = cx - rel_y
            py = cy - rel_x
            ...
            # Draw cyan circle (SLAM node)
            self.display.setColor(0x00FFFF)
            self.display.drawOval(px - 2, py - 2, 4, 4)

    def draw_slam_nodes_global(self, graph_map, slam_poses):
        """
        Draw SLAM keyframe nodes on global map (cyan circles).
        ...
        for pose in slam_poses:
            node_gx, node_gy = graph_map.world_to_grid(pose[0], pose[1])
            px = int(self._global_cx - node_gy * scale)
            py = int(self._global_cy - node_gx * scale)
            
            if not (0 <= px < self.gdisplay_w and 0 <= py < self.gdisplay_h):
                continue
            
            # Draw cyan circle (SLAM node)
            self.global_display.setColor(0x00FFFF)
            sz = max(2, int(scale))
            self.global_display.fillRectangle(px - sz, py - sz, sz * 2, sz * 2)
