...
class PerceptionSystem:
    ...
    def process_frame(self, frame_rgb):
        ...
        # ตรวจจับการเคลื่อนที่ด้วย Temporal Difference
        diff = np.abs(gray - self.prev_gray)
        motion_mask = diff > self.threshold_motion
...
# ==========================================================
# PART 2: GRAPH-BASED MAP (Requirement #1: Dictionary Map)
# ==========================================================
class GraphMap:
    def __init__(self, resolution=0.05):
        # Key: (gx, gy) grid coordinates, Value: probability [0, 1]
        self.nodes = {} 
        self.resolution = resolution
...
class MyRobot:
    def __init__(self, bot):
        ...
        self.display = self.supervisor.getDevice('display')
        if self.display:
            # ประกาศตัวแปรขนาดหน้าจอเพื่อแก้ AttributeError
            self.display_width = self.display.getWidth()
            self.display_height = self.display.getHeight()
            
        # เพิ่มตัวแปรเก็บจุดเริ่มต้นสำหรับทำ Relative Map
        self.start_gx = None
        self.start_gy = None
        ...
        # Navigation State
        self.mode = "TELEOP" # Modes: TELEOP, AUTO, EXPLORE (A*)
        self.current_path = []

    def get_pose(self):
        # Perfect Odometry via Supervisor
        pos = self.robot_node.getField("translation").getSFVec3f()
        rot = self.robot_node.getField("rotation").getSFRotation()
        yaw = -rot[3] if rot[2] > 0 else rot[3]
        return pos[0], pos[1], yaw
    ...
    def filter_moving_lidar(self, ray_angle, motion_mask):
        """ Requirement #2: ตรวจสอบว่า LiDAR ลำนี้ตรงกับพิกัดภาพที่ขยับหรือไม่ """
        if motion_mask is None: return False
        
        # ถ้า Ray อยู่ในมุมมองกล้อง
        if abs(ray_angle) < (self.cam_fov / 2):
            # แปลงมุมเป็นพิกัด Pixel X (0 ถึง width)
            pixel_x = int((0.5 - (ray_angle / self.cam_fov)) * self.cam_width)
            pixel_x = np.clip(pixel_x, 0, self.cam_width - 1)
            
            # เช็คใน motion_mask ว่าคอลัมน์นี้มีการขยับไหม
            if np.any(motion_mask[:, pixel_x]):
                return True # วัตถุกำลังเคลื่อนที่
        return False
    ...
    def draw_map(self):
        ...
        # 2. ดึงพิกัดหุ่นยนต์ปัจจุบันมาเป็น "ศูนย์กลางหน้าจอ"
        rx, ry, _ = self.get_pose()
        robot_gx, robot_gy = self.graph_map.world_to_grid(rx, ry)

        # 3. วาดกำแพงจากพิกัดใน Dictionary (สีดำ)
        self.display.setColor(0x000000)
        for (gx, gy), prob in self.graph_map.nodes.items():
            if prob >= 0.5:
                # คำนวณระยะห่างของกำแพงเทียบกับตำแหน่งหุ่นยนต์
                rel_x = gx - robot_gx
                rel_y = gy - robot_gy

                # 4. หมุนแกนหน้าจอให้ตรงกับ 3D View (สลับแกน X, Y ตามที่คุณต้องการ)
                px = center_x - (rel_y * pixels_per_cell) 
                py = center_y - (rel_x * pixels_per_cell)
                
                # วาดเฉพาะส่วนที่อยู่ในกรอบหน้าจอ Display
                if 0 <= px < self.display_width and 0 <= py < self.display_height:
                    self.display.fillRectangle(int(px), int(py), pixels_per_cell, pixels_per_cell)

        # 5. วาดเป้าหมาย Frontier (สีเขียว) ให้เลื่อนตามหุ่นยนต์ด้วย
        if hasattr(self, 'target_grid') and self.target_grid:
            rel_tx = self.target_grid[0] - robot_gx
            rel_ty = self.target_grid[1] - robot_gy
            tx = center_x - (rel_ty * pixels_per_cell)
            ty = center_y - (rel_tx * pixels_per_cell)
        ...
        # 6. วาดตัวหุ่นยนต์ (สีแดง) ให้อยู่ "ตรงกลางหน้าจอเสมอ"
        self.display.setColor(0xFF0000)
        self.display.fillRectangle(center_x-2, center_y-2, 4, 4)
    ...
    def handle_teleop(self):
        ...
        elif key == ord('M'): # กด M เพื่อสลับโหมด
            self.mode = "AUTO"
    ...
    def handle_auto(self):
        # ระบบ Random Walk จาก Milestone 2
        lidar_ranges = self.lidar.getRangeImage()
        front = min(lidar_ranges[len(lidar_ranges)//2-20 : len(lidar_ranges)//2+20])
    ...
    def run(self):
        print("Milestone 3: Controller Started")
        print("Controls: WASD=Move, M=Toggle Mode")
        
        while self.supervisor.step(self.time_step) != -1:
            # 1. Update Map (Dictionary + Fusion)
            self.mapping_logic()
            ...
            if self.mode == "TELEOP":
                ...
                # พื้นที่สำหรับใส่ A* และ Frontier ในอนาคต
                pass
