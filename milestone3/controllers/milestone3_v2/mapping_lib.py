...
class Mapper:
    ...
    def __init__(self, graph_map, occupancy_increment=0.05, decay_rate=0.01):
        """
        Args:
            graph_map: GraphMap instance
            occupancy_increment: How much to increase occupancy per wall detection
            decay_rate: How much to decrease stale occupancy per frame
        """
        self.graph_map = graph_map
        self._OCCUPANCY_INCREMENT = occupancy_increment
        self._DECAY_RATE = decay_rate
        self._last_observed_cells = set()
    ...
    def update_map(self, blocked_cols, lidar_ranges, rx, ry, ryaw, camera_hfov, cam_w, lidar_fov):
        ...
        for i, dist in enumerate(lidar_ranges):
            ...
            self.graph_map.add_to_map(wx, wy, increment=self._OCCUPANCY_INCREMENT)
            observed_cells.add(self.graph_map.world_to_grid(wx, wy))

        # Decay occupancy for cells not re-confirmed (reduces false positives)
        for cell in self._last_observed_cells:
            if cell not in observed_cells:
                if cell in self.graph_map.nodes:
                    self.graph_map.nodes[cell] = max(0.0, self.graph_map.nodes[cell] - self._DECAY_RATE)

        self._last_observed_cells = observed_cells
